...
const Catalog = () => {
  ...
  const handleAddToCart = async (userId, websiteId) => {
    try {
      await cartService.createCart(userId, websiteId);
  ...
  const toggleFavorite = async (websiteId, userId) => {
    try {
      if (favorites.has(websiteId)) {
        ...
        if (favorite) {
          ...
          await favouriteService.deleteFavourite(favorite._id, userId);
          setFavorites(prev => {
        ...
        await favouriteService.createFavourite(userId, websiteId);
        setFavorites(prev => {
          const newFavorites = new Set(prev);
          ...
          <div className="flex flex-wrap justify-between items-center mb-4">
            ...
            <div className="flex gap-3 mt-2 sm:mt-0">
              ...
              <button 
                ...
                onClick={() => toggleFavorite(item._id, item.userId)}
              >
              ...
              <button 
                ...
                onClick={() => handleAddToCart(item.userId, item._id)}
              >
                Add to Cart
              </button>
            </div>
